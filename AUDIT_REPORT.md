# FerryAI — Full Project Audit Report

**Date:** 2026-07-06
**Auditor:** Automated full-codebase review
**Scope:** Every PHP source file, every test file, every config file (425 files, 14 packages)
**Branch:** `main` (`bccfac4` at time of analysis)
...
|----------|-------|-------------|
| CRITICAL | 6 | Broken functionality, contract violations, always-failing code paths |
| HIGH | 7 | Significant bugs, misleading API, unimplemented features posing as complete |
| MEDIUM | 12 | Design issues, tooling problems, code duplication |
| LOW | 8 | Cosmetic, static analysis noise, minor inconsistencies |
...
**Files:**
- `packages/ai/src/AI.php:196-198` — `$config->get('backends.classify.model_path')`
- `packages/ai/src/AI.php:227-229` — `$config->get('backends.moderate.model_path')`
- `packages/ai/src/AI.php:262-264` — `$config->get('backends.predict.model_path')`

`AIConfig` defaults define `'backends' => []` (empty array). The keys `backends.classify.model_path`, `backends.moderate.model_path`, `backends.predict.model_path` do not exist in any default configuration, and the Laravel/Symfony service providers also define only `backends.onnx.*` and `backends.llama.*`.

**Result:** Calling `AI::classify()`, `AI::moderate()`, or `AI::predict()` **always throws `ConfigurationException`** unless the user manually adds undocumented config keys. These code paths are effectively dead unless the user has read the source code to discover the required keys.

**Fix:** Add to `AIConfig::defaults()`:
```php
'backends' => [
    'classify' => ['model_path' => ''],
    'moderate' => ['model_path' => ''],
